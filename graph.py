...
file_name = 'MR#129_2N000_4W_000deg_20250128_105714.txt' #''が絶対必要
f = np.loadtxt(file_name,delimiter='\t',) #なぜか区切り文字が'\t'、tab区切りを意味するらしい

a = np.array(f)

a_split = np.hsplit(a,5) #列方向(縦方向)に1列ごとに分割 分割数に注意
...
#ここからorigin用データ作成エリア
output_file_name = "si_mr_for_o.txt"
...
#plt.scatter(x, y, s=20, c='r', alpha=1, edgecolors='None') #散布図の作成
#plt.xlabel('B(mT)',fontsize=20) #texで斜体で打ちたい
#plt.ylabel('R(%)',fontsize=20)
#lt.tick_params(direction="in") # 目盛りの方向を内向きに設定
